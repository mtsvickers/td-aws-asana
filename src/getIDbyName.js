...
module.exports = (event, context, callback) => {
	...
	if( ! event.hasOwnProperty('data') ) {
		...
		if( searchStr.length <= 0 || ( typeof searchStr !== 'string' && ! searchStr instanceof String ) ) {
			...
			if( event.hasOwnProperty('type') && event.type.length > 0 ) {
				type = event.type;
			}
			if( event.hasOwnProperty('workspace') && ! isNaN( event.workspace ) ) {
